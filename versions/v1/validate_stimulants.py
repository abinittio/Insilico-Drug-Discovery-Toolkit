...
def predict_compound(smiles: str, models: dict) -> dict:
    ...
    if 'mat' in models:
        ...
        if dat_class == 'substrate':
            base = 75 + (dat_probs[2] * 20)
        elif dat_class == 'blocker':
            base = 65 + (dat_probs[1] * 25)
        else:
            base = 10 + (dat_active * 20)

        if sert_active > 0.01:
            selectivity = dat_active / (sert_active + 0.1)
            selectivity_bonus = min(selectivity * 5, 15)
        else:
            selectivity_bonus = 15

        score = min(100, max(0, base + selectivity_bonus))
        result['abuse_score'] = score
        result['abuse_category'] = 'HIGH' if score >= 60 else ('MODERATE' if score >= 35 else 'LOW')
