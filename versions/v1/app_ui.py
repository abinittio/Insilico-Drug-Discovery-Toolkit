...
def calculate_abuse_liability(dat_probs, net_probs, sert_probs, dat_class):
    """Calculate abuse liability score based on DAT activity.

    Key factors:
    - DAT activity (blocker or substrate) = high abuse potential
    - DAT selectivity over SERT = higher abuse potential (SERT blunts reward)
    - Probability/confidence of DAT activity
    """
    # Get active probabilities (blocker + substrate combined)
    dat_active = dat_probs[1] + dat_probs[2]  # blocker + substrate
    sert_active = sert_probs[1] + sert_probs[2]

    # Base score from DAT activity type and confidence
    if dat_class == 'substrate':
        # Releasers (amphetamine-like) - highest risk
        base = 75 + (dat_probs[2] * 20)  # 75-95 based on confidence
    elif dat_class == 'blocker':
        # Blockers (cocaine-like) - high risk
        base = 65 + (dat_probs[1] * 25)  # 65-90 based on confidence
    else:
        # Inactive at DAT - low risk
        base = 10 + (dat_active * 20)  # Could still have some activity

    # DAT/SERT selectivity bonus - SERT activity reduces euphoria
    # High DAT + Low SERT = more abuse potential
    if sert_active > 0.01:
        selectivity = dat_active / (sert_active + 0.1)
        selectivity_bonus = min(selectivity * 5, 15)
    else:
        selectivity_bonus = 15  # No SERT activity = max bonus
...
def predict_molecule(smiles: str, models: dict) -> dict:
    ...
    try:
        ...
        if 'mat' in models and models['mat'] is not None:
            ...
            # Abuse liability - pass full probability arrays
            ...
            score, category = calculate_abuse_liability(dat_probs, net_probs, sert_probs, dat_class)
